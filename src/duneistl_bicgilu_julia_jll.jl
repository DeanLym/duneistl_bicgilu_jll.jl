module duneistl_bicgilu_julia_jll
...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import duneistl_bicgilu_julia_jll on Julia versions older than 1.3!")
end
...
if isdir(override_dir)
    ...
    function find_artifact_dir()
        return artifact"duneistl_bicgilu_julia"
    end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("duneistl_bicgilu_julia_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "duneistl_bicgilu_julia_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"duneistl_bicgilu_julia", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "duneistl_bicgilu_julia_jll", "src", "duneistl_bicgilu_julia_jll.jl"))
        @info("duneistl_bicgilu_julia_ll dev'ed out to /home/yiminliu/.julia/dev/duneistl_bicgilu_julia_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("a9e5d5f4-2699-5525-b05c-0994d2f797c3"))
platforms = [Pkg.Artifacts.unpack_platform(e, "duneistl_bicgilu_julia", artifacts_toml) for e in artifacts["duneistl_bicgilu_julia"]]
...
if best_platform === nothing
    @debug("Unable to load duneistl_bicgilu_julia; unsupported platform $(triplet(platform_key_abi()))")
else
...
end  # module duneistl_bicgilu_julia_jll
